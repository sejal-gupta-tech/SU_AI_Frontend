...
import { Mic, Plus, MessageSquare, Trash2, ChevronLeft, ChevronRight, Settings } from "lucide-react";
...
export default function UniAIPage() {
  const [sessions, setSessions] = useState<ChatSession[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("uni_ai_sessions");
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("uni_ai_sessions", JSON.stringify(sessions));
    }
  }, [sessions]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let aiContent = "";
      let customAction = res.action;

      if (res.action === "chat" || !res.success) {
        aiContent = res.message || res.data || "Sorry, I couldn't understand that.";
        if (typeof aiContent === 'string' && aiContent.includes("Business profile not found")) {
          customAction = "setup_brand";
        }
      } else {
            ...
            : {
                ...
                messages: [
                  ...
                  { 
                    ...
                    role: "ai", 
                    content: errMsg,
                    action: typeof errMsg === 'string' && errMsg.includes("Business profile not found") ? "setup_brand" : undefined,
                  },
                ],
              }
        )
  ...
  const renderDataCard = (action: string, data: any) => {
    if (action === "setup_brand") {
      return (
        <div style={{ marginTop: 12 }}>
          <Link href="/brand" style={{ textDecoration: "none" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", padding: "8px 16px", borderRadius: 20, color: "#fff", fontWeight: 600, fontSize: 13, cursor: "pointer", transition: "all 0.2s" }}>
              <Settings size={14} />
              Setup Brand Kit
            </div>
          </Link>
